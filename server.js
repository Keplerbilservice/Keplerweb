...
const EPOST_FRA = (process.env.EMAIL_FROM_NAME || 'Kepler Bilservice') + ' <' + (process.env.EMAIL_FROM_ADDRESS || process.env.SMTP_FRA || 'kundeservice@kepler.no') + '>';
const ADMIN_EPOST = process.env.ADMIN_NOTIFICATION_EMAIL || process.env.VARSEL_EPOST || 'kundeservice@kepler.no';
...
function sendEpost(db, til, emne, tekst) {
  ...
  db.emails.push(post);
  if (transport && til) {
    transport.sendMail({ from: process.env.EMAIL_FROM_ADDRESS || process.env.SMTP_FRA || 'kundeservice@kepler.no', to: til, subject: emne, text: tekst })
...
// Kundedata: DB_PATH pekes til persistent disk i produksjon (Railway Volume / Render Disk) — ellers overlever ikke dataene en redeploy.
const DB_FIL = process.env.DB_PATH || path.join(__dirname, 'db.json');
...
// Daglig sikkerhetskopi ved siden av databasen — beholder de siste 30
function sikkerhetskopi() {
  try {
    if (!fs.existsSync(DB_FIL)) return;
    const mappe = path.join(path.dirname(DB_FIL), 'backups');
    fs.mkdirSync(mappe, { recursive: true });
    const navn = 'db-' + new Date().toISOString().slice(0, 10) + '.json';
    const maal = path.join(mappe, navn);
    if (!fs.existsSync(maal)) fs.copyFileSync(DB_FIL, maal);
    fs.readdirSync(mappe).filter(f => /^db-\d{4}-\d{2}-\d{2}\.json$/.test(f)).sort().slice(0, -30)
      .forEach(f => fs.unlinkSync(path.join(mappe, f)));
  } catch (e) { console.warn('Sikkerhetskopi feilet:', e.message); }
}
sikkerhetskopi();
setInterval(sikkerhetskopi, 6 * 3600 * 1000);
...
app.get('/api/kjoretoy/:regnr', (req, res) => {
  ...
  fetch('https://akfell-datautlevering.atlas.vegvesen.no/enkeltoppslag/kjoretoydata?kjennemerke=' + encodeURIComponent(regnr), {
    ...
    const data = {
      ...
      regDato: (k.forstegangsregistrering || {}).registrertForstegangNorgeDato || '',
      farge: pent(farge),
...
// ---------- Veiviser-regler (styrer anbefalingene for ALLE besøkende) ----------
app.get('/api/veiviser-regler', (req, res) => res.json(lesDb().veiviserRegler || null));
app.put('/api/veiviser-regler', (req, res) => {
  const db = lesDb();
  db.veiviserRegler = req.body && Object.keys(req.body).length ? req.body : null;
  loggfør(db, 'Veiviser-regler oppdatert');
  skrivDb(db);
  res.json({ ok: true });
});

// ---------- Vipps Logg inn (OIDC) ----------
// Nøkler legges inn i adminpanelet (Integrasjoner → Vipps Logg inn) eller som miljøvariabler.
// Testmodus (testbrukere) bruker Vipps' testmiljø apitest.vipps.no — standard PÅ til lansering.
function vippsOppsett() {
  const o = lesDb().vippsOppsett || {};
  return {
    id: o.clientId || process.env.VIPPS_CLIENT_ID || '',
    hemmelig: o.clientSecret || process.env.VIPPS_CLIENT_SECRET || '',
    test: o.testmodus !== undefined ? !!o.testmodus : process.env.VIPPS_TEST_MODE !== 'false'
  };
}
function vippsVert() { return vippsOppsett().test ? 'apitest.vipps.no' : 'api.vipps.no'; }
const vippsSesjoner = new Map(); // token/state -> data
function vippsBase(req) { return (req.headers['x-forwarded-proto'] || 'https') + '://' + req.headers.host; }
app.get('/api/vipps/status', (req, res) => { const v = vippsOppsett(); res.json({ konfigurert: !!(v.id && v.hemmelig), testmodus: v.test }); });
// Admin: les/lagre Vipps-nøkler. Secret maskeres ved lesing og overskrives kun når et nytt sendes inn.
app.get('/api/settings/vipps', (req, res) => {
  if (!krevAdminTidlig(req, res)) return;
  const o = lesDb().vippsOppsett || {};
  res.json({ clientId: o.clientId || '', harSecret: !!(o.clientSecret || process.env.VIPPS_CLIENT_SECRET), testmodus: o.testmodus !== undefined ? !!o.testmodus : true });
});
app.put('/api/settings/vipps', (req, res) => {
  if (!krevAdminTidlig(req, res)) return;
  const db = lesDb();
  const inn = req.body || {};
  const o = db.vippsOppsett || {};
  db.vippsOppsett = {
    clientId: inn.clientId !== undefined ? String(inn.clientId).trim() : (o.clientId || ''),
    clientSecret: inn.clientSecret ? String(inn.clientSecret).trim() : (o.clientSecret || ''),
    testmodus: inn.testmodus !== undefined ? !!inn.testmodus : (o.testmodus !== undefined ? !!o.testmodus : true)
  };
  loggfør(db, 'Vipps-oppsett oppdatert (testmodus: ' + (db.vippsOppsett.testmodus ? 'på' : 'av') + ')');
  skrivDb(db);
  res.json({ ok: true, harSecret: !!db.vippsOppsett.clientSecret, testmodus: db.vippsOppsett.testmodus });
});
app.get('/api/vipps/login', (req, res) => {
  const v = vippsOppsett();
  if (!v.id || !v.hemmelig) return res.status(503).send('Vipps client ID/secret mangler — legg inn i adminpanelet under Integrasjoner.');
  const state = crypto.randomBytes(16).toString('hex');
  vippsSesjoner.set('state:' + state, { tid: Date.now(), retur: String(req.query.retur || '/') });
  res.redirect('https://' + vippsVert() + '/access-management-1.0/access/oauth2/auth?client_id=' + encodeURIComponent(v.id)
    + '&response_type=code&scope=' + encodeURIComponent('openid name phoneNumber email')
    + '&state=' + state + '&redirect_uri=' + encodeURIComponent(vippsBase(req) + '/api/vipps/callback'));
});
app.get('/api/vipps/callback', async (req, res) => {
  try {
    const st = vippsSesjoner.get('state:' + req.query.state);
    if (!st) return res.status(400).send('Ugyldig eller utløpt innlogging. Gå tilbake og prøv igjen.');
    vippsSesjoner.delete('state:' + req.query.state);
    const v = vippsOppsett();
    const tokenSvar = await fetch('https://' + vippsVert() + '/access-management-1.0/access/oauth2/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', Authorization: 'Basic ' + Buffer.from(v.id + ':' + v.hemmelig).toString('base64') },
      body: 'grant_type=authorization_code&code=' + encodeURIComponent(req.query.code || '') + '&redirect_uri=' + encodeURIComponent(vippsBase(req) + '/api/vipps/callback')
    }).then(r => r.json());
    if (!tokenSvar.access_token) throw new Error(tokenSvar.error_description || tokenSvar.error || 'fikk ikke tilgangstoken');
    const info = await fetch('https://' + vippsVert() + '/vipps-userinfo-api/userinfo', { headers: { Authorization: 'Bearer ' + tokenSvar.access_token } }).then(r => r.json());
    const token = crypto.randomBytes(24).toString('hex');
    vippsSesjoner.set(token, { navn: info.name || '', tlf: String(info.phone_number || '').replace(/^0047|^47/, ''), epost: info.email || '', tid: Date.now() });
    // Innloggingslogg (vises i admin → Integrasjoner)
    try { const db = lesDb(); db.vippsLogg = [{ tid: new Date().toISOString(), navn: info.name || '', tlf: String(info.phone_number || '').slice(-8), retur: st.retur, testmodus: vippsOppsett().test }, ...(db.vippsLogg || [])].slice(0, 500); skrivDb(db); } catch (e) {}
    res.setHeader('Set-Cookie', 'kepler_vipps=' + token + '; HttpOnly; Path=/; Max-Age=3600; SameSite=Lax');
    res.redirect(st.retur);
  } catch (err) {
    const db = lesDb(); loggfør(db, 'Vipps-innlogging feilet: ' + err.message); skrivDb(db);
    res.redirect('/');
  }
});
app.get('/api/vipps/meg', (req, res) => {
  const m = (req.headers.cookie || '').match(/kepler_vipps=([^;]+)/);
  const b = m && vippsSesjoner.get(m[1]);
  if (!b || Date.now() - b.tid > 3600e3) return res.json({ innlogget: false });
  res.json({ innlogget: true, navn: b.navn, tlf: b.tlf, epost: b.epost });
});
// Admin: innloggingslogg og samtykke-innboks
app.get('/api/vipps/logg', (req, res) => { if (!krevAdminTidlig(req, res)) return; res.json(lesDb().vippsLogg || []); });
app.get('/api/vipps/samtykker', (req, res) => { if (!krevAdminTidlig(req, res)) return; res.json((lesDb().vippsSamtykker || []).filter(s => !s.slettet)); });
// Webhook fra Vipps: bruker trakk samtykket — legges i samtykke-innboksen i admin
app.post('/api/vipps/samtykke-trukket', (req, res) => {
  const db = lesDb();
  const inn = req.body || {};
  db.vippsSamtykker = [{ id: crypto.randomBytes(8).toString('hex'), tid: new Date().toISOString(), navn: inn.name || '', tlf: String(inn.phoneNumber || inn.phone_number || '').slice(-8), sub: inn.sub || '', slettet: false }, ...(db.vippsSamtykker || [])];
  loggfør(db, 'Vipps: bruker trakk samtykke — lagt i samtykke-innboksen');
  skrivDb(db);
  res.json({ ok: true });
});
// Admin: slett/anonymiser kundedata etter trukket samtykke (GDPR)
app.post('/api/vipps/samtykker/:id/slett', (req, res) => {
  if (!krevAdminTidlig(req, res)) return;
  const db = lesDb();
  const s = (db.vippsSamtykker || []).find(x => x.id === req.params.id);
  if (!s) return res.status(404).json({ error: 'Fant ikke samtykket.' });
  const tlf8 = String(s.tlf || '').slice(-8);
  const anonym = o => { if (o.customer && String(o.customer.phone || '').replace(/\D/g, '').slice(-8) === tlf8) { o.customer.name = 'Slettet kunde'; o.customer.phone = ''; o.customer.email = ''; } };
  (db.orders || []).forEach(anonym); (db.bookings || []).forEach(anonym);
  db.vippsLogg = (db.vippsLogg || []).filter(l => String(l.tlf || '').slice(-8) !== tlf8);
  s.slettet = true; s.navn = ''; s.tlf = '';
  loggfør(db, 'Vipps-data slettet/anonymisert etter trukket samtykke');
  skrivDb(db);
  res.json({ ok: true });
});

// ---------- Gjenkjenning av kunde (brukes etter Vipps-innlogging) ----------
// Slår opp telefonnummeret i ordre- og bookinghistorikken og returnerer sist brukte reg.nr.
app.get('/api/kunde', (req, res) => {
  const tlf = String(req.query.tlf || '').replace(/\D/g, '').slice(-8);
  if (tlf.length !== 8) return res.status(400).json({ error: 'Oppgi telefonnummer.' });
  const db = lesDb();
  const treff = [...(db.orders || []), ...(db.bookings || [])]
    .filter(o => String((o.customer || {}).phone || '').replace(/\D/g, '').slice(-8) === tlf)
    .sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
  const siste = treff[0];
  if (!siste) return res.json({ kjent: false });
  const regnr = ((siste.customer || {}).regnr) || siste.regnr || '';
  res.json({ kjent: true, navn: (siste.customer || {}).name || '', epost: (siste.customer || {}).email || '', regnr, antallBesok: treff.length });
});
...
// ---------- Sharebox (nøkkelskap) ----------
// Med SHAREBOX_API_URL satt gjøres ekte reservasjon; uten simuleres luke og SMS går via vanlig varsling.
const SHAREBOX_KEY = process.env.SHAREBOX_API_KEY || '';
const SHAREBOX_URL = (process.env.SHAREBOX_API_URL || '').replace(/\/$/, '');
app.get('/api/sharebox/status', (req, res) => res.json({ konfigurert: !!SHAREBOX_KEY, api: !!SHAREBOX_URL }));
app.post('/api/sharebox/reserver', async (req, res) => {
  const { type, tlf, reg, ordreRef } = req.body || {};
  const til = normaliserTlf(tlf);
  if (!til) return res.status(400).json({ error: 'Oppgi gyldig mobilnummer.' });
  const retning = type === 'innlevering' ? 'innlevering' : 'utlevering';
  const db = lesDb();
  if (SHAREBOX_URL && SHAREBOX_KEY) {
    try {
      const svar = await fetch(SHAREBOX_URL + '/reservations', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + SHAREBOX_KEY }, body: JSON.stringify({ phone: til, direction: retning, reference: ordreRef || reg || '' }) });
      if (!svar.ok) throw new Error('Sharebox svarte ' + svar.status);
      const data = await svar.json();
      loggfør(db, 'Sharebox: luke ' + (data.boxNumber || '?') + ' reservert (' + retning + ') for ' + til); skrivDb(db);
      return res.json({ ok: true, luke: data.boxNumber || null, kilde: 'sharebox' });
    } catch (e) {
      loggfør(db, 'SHAREBOX-FEIL: ' + e.message); skrivDb(db);
      return res.status(502).json({ error: 'Sharebox-reservasjonen feilet: ' + e.message });
    }
  }
  const luke = 1 + Math.floor(Math.random() * 16);
  varsle(db, 'sms', til, 'Sharebox', 'Kepler: Nokkelen ' + (retning === 'utlevering' ? 'ligger klar i' : 'legges i') + ' Sharebox-skapet, luke ' + luke + '. Aapne luken med mobilen. Ref ' + (ordreRef || reg || '') + '.');
  loggfør(db, 'Sharebox (simulert): luke ' + luke + ' reservert (' + retning + ') for ' + til); skrivDb(db);
  res.json({ ok: true, luke, kilde: 'simulert' });
});
...
const RUTER = {
  ...
  // Avpublisert (04.09.2026): /bilforhandler, /bat, /bobil, /bilsalg — legg tilbake her for å republisere (kommer da også inn i sitemap igjen)
...
app.get('/robots.txt', (req, res) => {
  res.type('text/plain').send('User-agent: *\nDisallow: /admin\nDisallow: /api\nDisallow: /bestill\nDisallow: /min-side\nSitemap: ' + DOMENE + '/sitemap.xml\n\n# AI-søk er velkommen på åpne sider\nUser-agent: GPTBot\nAllow: /\nDisallow: /admin\nDisallow: /api\nUser-agent: ClaudeBot\nAllow: /\nDisallow: /admin\nDisallow: /api\nUser-agent: PerplexityBot\nAllow: /\nDisallow: /admin\nDisallow: /api\n');
});
// ---------- GEO: strukturerte data + llms.txt (AI-søk: ChatGPT, Perplexity, Claude m.fl.) ----------
const JSONLD = JSON.stringify({ '@context': 'https://schema.org', '@type': 'AutoRepair', name: 'Kepler Bilservice',
  description: 'Bilpleie, rustbeskyttelse, keramisk coating og Smart Repair på Sem i Tønsberg. Siden 2004.',
  url: DOMENE, telephone: '+47 33 33 44 00', email: 'kundeservice@kepler.no',
  address: { '@type': 'PostalAddress', streetAddress: 'Andebuveien 63', postalCode: '3170', addressLocality: 'Sem', addressRegion: 'Vestfold', addressCountry: 'NO' },
  ...
  areaServed: ['Tønsberg', 'Sandefjord', 'Vestfold'], foundingDate: '2004', priceRange: 'kr 490–11 990' });
function seoHode(tittel, beskrivelse, rute) {
  return '<title>' + tittel + '</title>\n<meta name="description" content="' + beskrivelse + '">\n'
    + '<link rel="canonical" href="' + DOMENE + rute + '">\n'
    + '<meta property="og:title" content="' + tittel + '">\n<meta property="og:description" content="' + beskrivelse + '">\n'
    + '<meta property="og:url" content="' + DOMENE + rute + '">\n<meta property="og:type" content="website">\n'
    + '<meta property="og:site_name" content="Kepler Bilservice">\n<meta property="og:locale" content="nb_NO">\n'
    + '<script type="application/ld+json">' + JSONLD + '</script>\n';
}
app.get('/llms.txt', (req, res) => {
  res.type('text/plain').send([
    '# Kepler Bilservice', '',
    'Bilpleie, rustbeskyttelse, keramisk coating og Smart Repair på Sem i Tønsberg (Vestfold, Norge). Etablert 2004.', '',
    'Adresse: Andebuveien 63, 3170 Sem · Telefon: 33 33 44 00 · E-post: kundeservice@kepler.no', 'Åpningstider: mandag–fredag 07.30–17.00', '',
    '## Tjenester (faste priser)',
    '- Antirust/understellsbehandling: Fluid Film Express, Fluid Film Pluss og Kepler Premium (inntil 16 års garanti). Gratis rustsjekk.',
    '- Lakkbeskyttelse: lakkforsegling, keramisk coating (inntil 5 års garanti), felgcoating, glasscoating, solfilm.',
    '- Bilpleie: håndvask, helvask, innvendig rens, skinnbehandling, lakkrens og polering.',
    '- Smart Repair: bulkfjerning uten lakkering (PDR), Spot Paint, felgreparasjon, lyktrenovering.',
    '- Bedrift: vaskeavtaler, klargjøring, faste avtaler.', '',
    '## Sider', Object.keys(RUTER).filter(p => p !== '/bestill' && p !== '/min-side').map(p => '- ' + DOMENE + p + ' — ' + RUTER[p][0]).join('\n')
  ].join('\n'));
});
...
Object.keys(RUTER).forEach(rute => {
  ...
  app.get(rute, (req, res) => {
    ...
    html = html.replace('</head>', seoHode(tittel, beskrivelse, rute) + '<script>if(!location.hash)location.hash=' + JSON.stringify(rute) + ';</script>\n</head>');
    res.send(html);
  });
});
...
app.get('/', (req, res) => {
  ...
  html = html.replace('</head>', seoHode(tittel, beskrivelse, '/') + '</head>');
  res.send(html);
});
